...
export default function LearnerReportCard() {
  ...
  // Updated grade description based on your grading system
  ...
  const downloadPDF = () => {
    ...
    try {
      ...
      doc.text(`Form: ${selectedReport?.form || 'N/A'}`, pageWidth - 85, 92);
  ...
  const downloadAllReports = () => {
    ...
    try {
      ...
      reports.forEach((report, index) => {
        ...
        doc.text(`Form: ${report.form || 'N/A'}`, 20, 106);
                ...
                <div className="bg-gradient-to-r from-[#1A237E] to-[#0D1240] text-white p-8">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                      ...
                      <p className="text-white/60 text-sm">{selectedReport.term} · {selectedReport.form}</p>
                    </div>
